 ...
 *
 */
Dragonfly_Module::Dragonfly_Module(MODULE_ID ModuleID, HOST_ID HostID, int use_time_card)
{
  TRY {
    InitVariables( ModuleID, HostID, use_time_card);
  } CATCH_and_THROW( "Dragonfly_Module::Dragonfly_Module( MODULE_ID ModuleID, HOST_ID HostID, int use_time_card)");
}

/*
 *
 */
void Dragonfly_Module::InitVariables( MODULE_ID ModuleID, HOST_ID HostID, int use_time_card)
{
  TRY {
    _pipeClient = NULL;
    _MMpipe = NULL;
...
Dragonfly_Module::SendMessage( CMessage *M, UPipe *output_pipe, MODULE_ID dest_mod_id, HOST_ID dest_host_id)
{
  TRY {
    ...
    {
      PCPS_HR_TIME hrt;
      int rc = fnc_mbg_get_xhrt_time_as_pcps_hr_time(&m_mbg_poll_thread.xhrt_info, &hrt);
      if (rc == MBG_SUCCESS)
      {
        M->utc_seconds  = hrt.tstamp.sec;
