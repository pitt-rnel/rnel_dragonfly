function status = ConnectToMMM( ModuleID, Reserved, MessageConfigFile, varargin)
    % status = ConnectToMMM( ModuleID, Reserved, MessageConfigFile, option1, option2, ...)
    %
    ...
    for i = 1 : length( varargin)
        ...
        switch( option_name)
            case '-logger' 
                 logger_status = 1;
            case '-server_name'
                 if ( length( option_values) ~= 1) 
                      error( '-server_name option must be specified with a server name in the form hostname:portnumber'); 
                 end
                 ServerName = option_values{1};
            otherwise
                 error( ['Unknown option name "' option_name '"']);
        end %switch
    end %for
    ...
    if ( status == 0) 
         error( 'Could not connect to MessageManager'); 
    end
