      ...
      switch( opcode) {
        ...
        case CONNECT_TO_MMM:
          ...
          if ( num_input_args < 4 )  
            ...
            use_time_card = (int) mxGetScalar( input_arg[4] );
          ...
          TheModule.InitVariables( ModuleID, 0, use_time_card );
          try{
            if ( HostAddressLength > 0 ) {
              // Convert unicode matlab string to regular character string
              if ( HostAddressLength > MAX_HOST_ADDR_LENGTH-1 ) 
                Error( "Server name exceeds maximum allowed length");
              HostAddress_raw = (short*) mxGetData( input_arg[2]);
              int i;
              for ( i = 0; i < HostAddressLength; i++ ) 
                HostAddress[i] = (char) HostAddress_raw[i];
